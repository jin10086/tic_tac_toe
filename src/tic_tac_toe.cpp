...
bool is_valid_movement(const uint16_t &row, const uint16_t &column, const std::vector<uint8_t> &board)
{
    ...
    bool is_valid = column < tic_tac_toe::game::board_width && movement_location < board.size() && is_empty_cell(board[movement_location]);
    return is_valid;
}
...
name get_winner(const tic_tac_toe::game &current_game)
{
    ...
    std::vector<uint32_t> consecutive_column(tic_tac_toe::game::board_width, 5);
    std::vector<uint32_t> consecutive_row(tic_tac_toe::game::board_height, 5);
    uint32_t consecutive_diagonal_backslash = 5;
    uint32_t consecutive_diagonal_slash = 5;
